...
int Server_Controller::Register(Packet * new_packet) {
  ...
  for (i=0; i < m_num_packets_reg; i++) {
	...
	if ((new_packet->GetType() != m_packet_types[i]) ||
	  ...
	  SET_RETURN_ERROR(PACKET_DEF_CLASH, PACKET_DEF_CLASH, (char *)"Register",
			   (char *)"Packet registration size and type does not match input packet");
	}
	else {
	  // matches
  ...
  if (m_num_packets_reg >= MAX_PACKET_TYPES) {
    ...
    SET_RETURN_ERROR(OUT_OF_PACKET_TYPES, OUT_OF_PACKET_TYPES, (char *)"Register",
		     (char *)"Ran out of Packet Types, increase MAX_PACKET_TYPES");
  }
  else {
...
int Server_Controller::StartServer(short port) {
  ...
  if ((m_listening_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
    SET_RETURN_ERROR(SOCKET_ERR, -201, (char *)"StartServer", 
  ...
  if ((setsockopt(m_listening_fd, SOL_SOCKET, SO_REUSEADDR, (char *)&on, sizeof(int)) == -1) ||
    ...
    SET_RETURN_ERROR(SOCKET_ERR, -202, (char *)"StartServer",
  ...
  if (bind(m_listening_fd, (struct sockaddr *)&server_addr, sockAddrSize) == -1) {
    close(m_listening_fd);
    m_listening_fd = -1;
    SET_RETURN_ERROR(SOCKET_ERR, -203, (char *)"StartServer",
		     (char *)"Bind call failed");
  }
  ...
  if (listen(m_listening_fd, 1) == -1) {
    close(m_listening_fd);
    m_listening_fd = -1;
    SET_RETURN_ERROR(SOCKET_ERR, -204, (char *)"StartServer",
		     (char *)"Listen call failed");
  }
...
int Server_Controller::Receive(BOOL wait) {
  ...
  if (m_client_fd >= 0) {
    ...
    // no client connected
    SET_RETURN_ERROR2(NO_CLIENT, NO_CLIENT, (char *)"Receive",
		      (char *)"No client to read from");
  }
}
...
int Server_Controller::Send() {
  ...
  if (result == 1) {
    if (m_client_fd >= 0) {
      ...
      SET_RETURN_ERROR2(NO_CLIENT, NO_CLIENT, (char *)"Send",
			(char *)"No client to send to");
    }
  }
  ...
  if (result == 1) {
    ...
    if (result != 1) {
      ...
      SET_RETURN_ERROR(REGISTRATION_ERROR, result, 
		       (char *)"CheckValidRegistration",
		       (char *)"Error in Add_Packet");
    }
	...
	fprintf(stderr, "Server_Controller (CheckValidRegistration) : Registered %s packet\n", reg_class_name);
      ...
      SET_RETURN_ERROR(REGISTRATION_ERROR, temp_error_num, 
		       (char *)"CheckValidRegistration",
		       (char *)"Received an unknown or wrong size registration packet");
    }
  }
  else {
    ...
    SET_RETURN_ERROR(REGISTRATION_ERROR, result, 
		     (char *)"CheckValidRegistration",
		     (char *)"Error in GetPacketFromMsg");
  }
}
...
int Server_Controller::CheckForNewClient() {
  ...
  if (FD_ISSET(m_listening_fd, &readfds)) {
    ...
    if ((m_client_fd = accept(m_listening_fd, (struct sockaddr *)&clientAddr, (socklen_t*)&sockAddrSize)) == -1) {
      m_client_fd = -1;
      perror(" accept ");
      SET_RETURN_ERROR(SOCKET_ERR, -205, (char *)"CheckForNewClient",
		       (char *)"Error in accept call");
    }
