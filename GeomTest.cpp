...
void GeomTest::init(bool OSG){
  ...
  for(int k=0;k<nz;k++){
	...
	data[tidx] = pc->tau[idx].t11;
	data[tidx+1] = pc->tau[idx].t22;
	data[tidx+2] = pc->tau[idx].t33;
	data[tidx+3] = pc->tau[idx].t13;
      }
    }
  }
  ...
  glTexParameteri(texType2, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexParameteri(texType2, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
  glTexImage3D(texType2, 0, int_format, nx, ny, nz,0,GL_RGBA, GL_FLOAT,data); 
...
void GeomTest::readInTables(){
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, int_format, 256,5,0,GL_RGBA, GL_FLOAT,data2); 
...
int GeomTest::display(){
  ...
  for(int z = 0; z <= nz; z++){
    ...
    float slice = ((float)(z+0.5f) / (float)(nz));
  ...
  for(int k=0; k < nz; k++){
    ...
    //glBindBufferBaseNV(GL_TRANSFORM_FEEDBACK_BUFFER_NV,0,iso_buffer[k]);
	...
	glVertex4f((float)j,(float)i,(float)k,1.0);
  ...
  gluLookAt(150.0,2.0,2.0, 1.0,2.0,2.0, 0.0, 0.0, 1.0);
  ...
  glBegin(GL_LINES);
  {
    glVertex3f(43.0,0.0,1.0);
    glVertex3f(43.0,ny,1.0);
  }
  glEnd();*/
  ...
  float r = 0.0/((float)(nz));

  glBindTexture(texType2,tex3d[0]);
  
  glBegin(GL_QUADS);
  {
    glTexCoord3f(0.0f, 0.0f, r);	        glVertex3f(0.0f, -2, -2.0f);
    glTexCoord3f(1.0f, 0.0f, r);		glVertex3f(0.0f, 2, -2.0f);
    glTexCoord3f(1.0f, 1.0f, r);	        glVertex3f(0.0f,  2, 2.0f);
    glTexCoord3f(0.0f, 1.0f, r);		glVertex3f(0.0f,  -2, 2.0f);
  }
  glEnd();  
  */
