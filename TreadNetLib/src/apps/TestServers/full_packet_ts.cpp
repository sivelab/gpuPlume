...
extern char* stairs_dir_string[3];
extern char* stairs_cmd_string[4];
...
extern Stairs_Packet * Stairs_pkt;
extern int Stairs_type;
extern Tracker_Packet * Tracker_pkt;
extern int ByteArray_type;
extern ByteArray_Packet * ByteArray_pkt;
...
int vxmain(int arg1, int arg2) {
  ...
  // Tracker Data (make up down left right keys affect position, 
  //   'y','p','r' affect quat data)
  TRPoint tracker_pos(0,0,-1);
  TRQuaternion tracker_orient;
  double tracker_thetax = 0;
  double tracker_thetay = 0;
  double tracker_thetaz = 0;
  t_data.SetPos(tracker_pos);
  t_data.SetOrientation(tracker_orient);
  
  // FSR Data (just simulate up down movement, constantly as if marching)
  fsr_data.SetValueRightHeel(0);
  fsr_data.SetValueLeftHeel(0);
  fsr_data.SetValueRightToe(0);
  fsr_data.SetValueLeftToe(0);
  // Create a pattern - from -10 V to 10 V back to -10 V back to 10 V (last half so can
  //  have all 4 sensors in pattern at all times)
  // just have right heel going first, right toe a bit behind,
  //  left heel half the cycle behind and left toe just a bit behind left heel
  int step_seq = 32; // step cycle on (i.e. right heel is down) 
  int step_skip = (int)(hz_request/10.0); // one foot step per 3 second
  int step_skip_count = 0;
  float fsr_values[96];
  for (int i=0; i < 96; i++) {
    if (i < 32) {
      fsr_values[i] = 20.0/32.0*(float)i - 10.0;
    }
    else if (i < 64) {
      fsr_values[i] = 10.0 - 20.0/32.0*((float)i-32.0);
    }
    else {
      fsr_values[i] = 20.0/32.0*(float)(i-64) - 10.0;
    }      
  }
  ...
  int rec_chararray = 0; // for char array
  ...
  while(!quit) {
    ...
    case KEY_RIGHT:
      ...
      // head tracker keys - yaw, pitch, roll 
      //  pitch is head nod (up down)
      //  roll is head tilt (side to side)
      //  yaw is head twist
    case 'Y':
      tracker_thetaz += .1; // rotate about z axis
      break;
    ...
    case 'P':
      tracker_thetay += .1; // rotate about y axis
      break;
    ...
    case 'R':
      tracker_thetax += .1; // rotate about x axis
      break;
    ...
    // update tracker info
    // recalculate tracker orientation (just treating each as euler angles and combining)
    tracker_orient = CreateQuatFromRotVec(tracker_thetax, TRVector(1,0,0)) *
      CreateQuatFromRotVec(tracker_thetay, TRVector(0,1,0)) * 
      CreateQuatFromRotVec(tracker_thetaz, TRVector(0,0,1));
    t_data.SetPos(tracker_pos);
    t_data.SetOrientation(tracker_orient);
    
    // update fsr signals
    step_skip_count++;
    if (step_skip_count >= step_skip) {
      step_skip_count = 0;
      step_seq++;
      if (step_seq > 95) {
	step_seq = 32;
    ...
    fsr_data.SetValueRightHeel(fsr_values[step_seq]);
    fsr_data.SetValueRightToe(fsr_values[step_seq-1]);
    fsr_data.SetValueLeftHeel(fsr_values[step_seq-31]);
    fsr_data.SetValueLeftToe(fsr_values[step_seq-32]);
	...
	if (packet_type == ServerRequest_type) {
	  ...
	  if (ServerRequest_pkt->IsPacketRequested(Fsr_pkt)) {
	    Fsr_pkt->SetData(&fsr_data);
	    if (server->AddPacket(Fsr_pkt) != 1) {
	      // some error in add packet, print it
	      server->PrintLastError();
	    }
	  }
	  if (ServerRequest_pkt->IsPacketRequested(Tracker_pkt)) {
	    Tracker_pkt->SetData(&t_data);
	    if (server->AddPacket(Tracker_pkt) != 1) {
	      // some error in add packet, print it
	      server->PrintLastError();
	    }
	  }  
	...
	else if (packet_type == Stairs_type) {
	  server->GetPacketFromMsg(Stairs_pkt);
	}
	else if (packet_type == ByteArray_type) {
	  rec_chararray = 1;
	  server->GetPacketFromMsg(ByteArray_pkt);
	}
	else {
    ...
    if (received_virtstate) {
      ...
      printw4(" Virtual Force:       %7.2lf X  %7.2lf Y  %7.2lf Z\n",
	     vf.X(),vf.Y(),vf.Z());
      printw3(" Stairs (Dir, Cmd):    %10s %10s\n",
	      stairs_dir_string[Stairs_pkt->GetStairsDir()], 
	      stairs_cmd_string[Stairs_pkt->GetCmd()]);
    }
    else {
    ...
    if (rec_chararray) {
      rec_chararray = 0;
      printw2(" Received ByteArray: %s\n",
	      ByteArray_pkt->GetArrayPtr());
      char * temp = ByteArray_pkt->GetArrayPtr();
      printw2(" Last element is: %c\n",
	      temp[ByteArray_pkt->GetLastElementInd()]);
    }
    else {
      printw1(" Received ByteArray: \n");
    }
