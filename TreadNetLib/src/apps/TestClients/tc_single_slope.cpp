...
double UpdateVW(const UserState_Data &u, VirtualState_Data &v, int slope_ang);
...
int vxmain(int arg1, int arg2, int arg3, int arg4) {
  ...
  double elevation;
  elevation = UpdateVW(us_data,vs_data,slope_angle);
  ...
  while(!quit) {
	...
	tmpfacing /= invnorm;
      ...
      tmp_vsd = vs_data;
      tmp_elev = UpdateVW(tmp_usd,tmp_vsd,slope_angle);
...
double UpdateVW(const UserState_Data &u, VirtualState_Data &v, int slope_ang) {
  ...
  // this is wrong!!or it's sent wrong???
