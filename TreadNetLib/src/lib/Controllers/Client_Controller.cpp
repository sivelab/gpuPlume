...
int Client_Controller::Register(Packet * new_packet) {
  ...
  if (MessageInUse() || m_num_packets_sending != 0) {
    SET_RETURN_ERROR(MSG_BUF_BUSY, MSG_BUF_BUSY, "Register",
  ...
  if (check_packet_type != 0) {
    ...
    Cancel_Message();
    SET_RETURN_ERROR(REGISTRATION_ERROR, -305, "Register",
		     "Returned packet from Registration was wrong type");
  }
  ...
  if (msg_end != -1) {
    ...
    delete returned_reg;
    SET_RETURN_ERROR(REGISTRATION_ERROR, -305, "Register",
		     "More than one packet was sent back w/ Registration");
  }
  ...
  if (result == 1) {
	...
	!(ret_class_size == new_packet->GetSize())) {
      ...
      delete returned_reg;
      SET_RETURN_ERROR(REGISTRATION_ERROR, -306, "Register",
		       "Wrong registration class/size sent back");
    }
    ...
    case INVALID_REG_SIZE:
      SET_RETURN_ERROR(PACKET_DEF_CLASH, PACKET_DEF_CLASH, "Register",
		       "Server has a different size for this packet class");
      break;
    case UNKNOWN_CLASS:
      SET_RETURN_ERROR(SERVER_REG_UNKNOWN, SERVER_REG_UNKNOWN, "Register",
		       "Server does not recognize this packet class name");
      break;
    default:
      // sent back some unknown tag
      SET_RETURN_ERROR(REGISTRATION_ERROR, -307, "Register",
		       "Server sent back an unknown Registration Tag Value");
    } 
  }
  else {
...
int Client_Controller::ConnectToServer(char * hostname, short port) {
  ...
  if ((m_server_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
    ...
    SET_RETURN_ERROR(SOCKET_ERR, -301, "ConnectToServer", 
		     "Socket call failed");
  }
  ...
  if (setsockopt(m_server_fd, IPPROTO_TCP, TCP_NODELAY, (char *)&on, sizeof(int)) == -1) {
    ...
    SET_RETURN_ERROR(SOCKET_ERR, -302, "ConnectToServer",
  ...
  if (connect(m_server_fd, (struct sockaddr *)&server_addr, sizeof(server_addr)) == -1) {
    ...
    SET_RETURN_ERROR(NO_SERVER, -303, "ConnectToServer",
		     "Connect call failed - nothing to connect to");
  }
...
int Client_Controller::AddPacket(Packet * new_packet) {
  if (MessageInUse()) {
    SET_RETURN_ERROR(MSG_BUF_BUSY, MSG_BUF_BUSY, "AddPacket",
		     "Message buffer is busy");
  }
...
int Client_Controller::Receive(BOOL wait) {
  ...
  if (m_server_fd < 0) {
    ...
    if (!TryReconnectToServer())
      SET_RETURN_ERROR2(NO_SERVER, NO_SERVER, "Receive",
  ...
  while (num_tries < m_num_receive_tries) {
	...
	Cancel_Message();
	SET_RETURN_ERROR(INCORRECT_RESPONSE_TAG, INCORRECT_RESPONSE_TAG, "Receive",
			 "Tag from response message did not match outgoing tag - incorrect response");
      }
      return 1;
      break;
    }
  ...
  if (wait) {
    ...
    SET_RETURN_ERROR(RECEIVE_TIMEOUT, RECEIVE_TIMEOUT, "Receive",
		     "Did not receive a message in the time allotted");
  }
  else {
...
int Client_Controller::Send() {
  ...
  if (m_server_fd < 0) {
    ...
    if (!TryReconnectToServer()) {
      ...
      Cancel_Message();
      SET_RETURN_ERROR2(NO_SERVER, NO_SERVER, "Send",
		       "Not connected to a server");
    }
  }
  ...
  if (result == MSG_ERROR) {
    // Close connection
    CloseServerConnection();
    SET_RETURN_ERROR(MSG_ERROR, MSG_ERROR, "Send",
		     "Server Connection Lost");
  }
  return result;
}
...
int Client_Controller::ReceiveSinglePacket(Packet * rec_pkt) {
  if (Receive(TRUE) == 1) {
	...
	SetErrorMessage("ReceiveSinglePacket","Received incorrect packet type");
	if (m_tread_verbosity) {
	  fprintf(stderr, "%s\n", m_error_msg);
	}
      }
...
int Client_Controller::TryReconnectToServer() {
  ...
  if (m_server_name == NULL || m_server_port < 0) {
    ...
    SET_RETURN_ERROR(0, -304, "TryReconnectToServer",
		     "No server/port specified to try and reconnect to");
  }
