...
int Net_Controller::GetPacketFromMsg(Packet * return_packet) {
  ...
  if (packet_type != return_packet->GetType()) {
    SET_RETURN_ERROR(PACKET_ERROR, -101, "GetPacketFromMsg",
		     "Input packet/message type mismatch");
  }
  ...
  if (packet_type != 0) {
    ...
    if (packet_size + m_msg_pointer > m_complete_message_size) {
      ...
      m_new_message = FALSE;
      SET_RETURN_ERROR(MSG_BUF_OVERRUN, MSG_BUF_OVERRUN, "GetPacketFromMsg",
  ...
  SET_RETURN_ERROR(PACKET_ERROR, -102, "GetPacketFromMsg",
		   "Packet Read Error");
}
...
int Net_Controller::PutPacketInMsg(Packet * packet_add) {
  if (m_new_message) {
    SET_RETURN_ERROR(MSG_BUF_BUSY, MSG_BUF_BUSY, "PutPacketInMsg", 
		     "Message Buffer Busy");
  }
  ...
  if (packet_add != NULL && packet_add->IsRegistered()) {
    ...
    if (m_msg_pointer + ind_packet_size + 1 > MESSAGE_BUFFER_SIZE) {
      ...
      SET_RETURN_ERROR(MSG_BUF_OVERRUN, MSG_BUF_OVERRUN, "PutPacketInMsg", 
    ...
    if (fill_size != ind_packet_size) {
      ...
      SET_RETURN_ERROR(PACKET_ERROR, -104, "PutPacketInMsg", 
		       "Packet's fill command error - incorrect fill bytes");
    }
    ...
    // invalid
    SET_RETURN_ERROR(PACKET_ERROR, -103, "PutPacketInMsg", 
		     "Unregistered Packet or Null Packet");
  }
  return 1;
}
...
int Net_Controller::Send_Packets(int fd, ushort tag) {
  ...
  if (m_new_message) {
    SET_RETURN_ERROR(MSG_BUF_BUSY, MSG_BUF_BUSY, "Send_Packets", "Message buffer busy");
  }
  ...
  if (m_msg_pointer > 4) {
    ...
    if (send_result != message_size) {
      ...
      SET_RETURN_ERROR(MSG_ERROR, MSG_ERROR, "Send_Packets", "Send error - incorrect bytes sent");
    }
...
int Net_Controller::Receive_Message(int fd, int waittime_usec) {
  ...
  if (m_new_message) {
    SET_RETURN_ERROR(MSG_BUF_BUSY, MSG_BUF_BUSY, 
		     "Receive_Message", "Message Buffer Busy");
  }
  ...
  while (1) {
    ...
    if (readresult <= 0) {
      ...
      SET_RETURN_ERROR(MSG_ERROR, MSG_ERROR, "Receive_Message", 
		       "Read from file descriptor failed");
    }
    else {
      // received some bytes
